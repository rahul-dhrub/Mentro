...
export default function BlogDetailPage() {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/blogs/${user_id}/${blog_id}`);
        ...
        if (response.ok) {
          ...
          const mockBlog = mockBlogs.find((b) => b.id === blog_id);
          
          if (mockBlog) {
            setBlog(mockBlog);
            setRelatedBlogs(mockBlogs.filter((b) => b.id !== blog_id).slice(0, 2));
          } else {
  ...
  const fetchRelatedBlogs = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        return data.blogs.filter((b: any) => b._id !== params.blog_id).slice(0, 2);
      }
    } catch (error) {
    ...
    // Fall back to mock data
    return mockBlogs.filter((b) => b.id !== params.blog_id).slice(0, 2);
  };
  ...
  if (error || !blog) {
    return (
      <Background>
        <BlogError error={error} userId={params.user_id as string} />
      </Background>
    );
  }
